...
dict1 = {char: i for i, char in enumerate(ascii_uppercase + string.digits)}
...
# Define the characters to use: lowercase, uppercase, and digits
...
def main(text):
    # Print each permutation one at a time
    with open("results_beaufort.txt", "a") as file:
        for perm in permutations:
            key = generate_key(text, ''.join(perm))
            file.write(original_text(text, key))
            file.write(" | ")
